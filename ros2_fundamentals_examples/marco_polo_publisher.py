...
class Marco_poloPublisher(Node):
    ...
    def __init__(self):
        ...
        super().__init__('marco_polo_publisher')
 
        # Creates a publisher on the topic "topic" with a queue size of 10 messages
        self.publisher_1 = self.create_publisher(String, '/marco_polo', 10)
...
def main(args=None):
    ...
    marco_polo_publisher = Marco_poloPublisher()
 
    # Keep the node running and processing events.
    rclpy.spin(marco_polo_publisher)
 
    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    marco_polo_publisher.destroy_node()
